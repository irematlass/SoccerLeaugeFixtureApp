            ...
            fun drawFixture(myTeamList:ArrayList<Team>):List<Fixture>{
               val fixtureList= mutableListOf<Fixture>()
                ...
                for (day in 1 until countOfWeek ){
                    ...
                    fixtureList.add(Fixture(UUID.randomUUID().toString(),tempTeams[teamIdx].name, myTeamList[0].name,day))
                    for (idx in 1 until countOfDay ) {
                        var firstTeam = (day + idx) % tempSize
                        var secondTeam = (day + tempSize - idx) % tempSize
                        fixtureList.add(Fixture(UUID.randomUUID().toString(),tempTeams[firstTeam].name, tempTeams[secondTeam].name,day))
                    }
                    }
